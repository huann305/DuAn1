...
public class CustomerDAO {
    ...
    public boolean insert(Customer object) {
        ...
        String sql = "INSERT INTO " + DBHelper.TABLE_CUSTOMER + " VALUES(?,?,?,?,?,?)";
    ...
    private List<Customer> getData(String sql, String... selectionArgs) {
        ...
        while (c.moveToNext()) {
            list.add(new Customer(c.getString(0), c.getString(1), c.getString(2), c.getString(3), c.getString(4), c.getString(5), c.getString(6)));
        }
        if (c != null) {
            c.close();
        }
